...
const SCALE_WIN   = 4;    // pause-window 140×110 → 560×440
const SCALE_BAR   = 2;    // scrollbars → 240×20
const SCALE_X     = 2.5;  // X button → 50×50
const SCALE_TEXT  = 1.5;  // text sprites → 36px tall
...
export default class PauseMenu extends Phaser.Scene {
    ...
    init(data) {
        this._callerKey = data?.callerKey ?? null;
    }
    ...
    create() {
        ...
        if (this._callerKey) this.scene.pause(this._callerKey);

        // Overlay
        this.add.rectangle(cx, cy, this.scale.width, this.scale.height, 0x000000, 0.4)
            .setInteractive();

        this.add.image(cx, cy, 'pause_window').setScale(SCALE_WIN);

        // X button 
        const xBtn = this.add.image(914, 130, 'opt_x')
            .setScale(SCALE_X)
        ...
        // ─ Layout constants
        const BAR_HALF   = (120 * SCALE_BAR) / 2;  // 120px
        const CHECK_HALF = 10;
        const TRACK_CX   = 900 - BAR_HALF;          // 780
        const CHECK_X    = TRACK_CX - BAR_HALF - 15 - CHECK_HALF;  // 635
        const LABEL_X    = 390;

        const ROW_SFX_Y = cy - 190;  // 278
        const ROW_MUS_Y = cy - 140;        // 368
        const ROW_FS_Y  = cy - 90;  // 453
        ...
        const fsBtn = this.add.image(TRACK_CX + BAR_HALF - CHECK_HALF, ROW_FS_Y,
                this._isFs ? 'opt_check_on' : 'opt_check_off')
        ...
        const returnBtn = this.add.image(cx, cy + 310, 'opt_return_to_menu')
            .setScale(3)
            .setInteractive({ useHandCursor: true });
        this._addHover(returnBtn);
        returnBtn.on('pointerdown', () => {
            this.scene.stop('hud');
            if (this._callerKey) this.scene.stop(this._callerKey);
